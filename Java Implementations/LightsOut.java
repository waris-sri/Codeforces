      ...
      for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
          ...
          if (presses[i][j] % 2 != 0) {
            ...
            if (i - 1 >= 0) lights[i - 1][j] = 1 - lights[i - 1][j]; // top neighbor
            if (j - 1 >= 0) lights[i][j - 1] = 1 - lights[i][j - 1]; // left neighbor
            if (i + 1 < 3) lights[i + 1][j] = 1 - lights[i + 1][j]; // bottom neighbor
            if (j + 1 < 3) lights[i][j + 1] = 1 - lights[i][j + 1]; // right neighbor
          }
        }
      }
      for (int i = 0; i < 3; i++) {
